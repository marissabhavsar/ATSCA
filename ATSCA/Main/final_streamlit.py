...
client = methods.set_up_ChatGPT_streamlit(calendar, tasks_list)  # set up the ChatGPT model
...
instruction = """\nAs my trusted virtual assistant, you are an expert in adeptly managing my calendar and assisting with scheduling tasks. Here is what I need from you: 1. **Calendar Management**:
...
user_request = st.text_input("Enter your query here", key="user_input")
...
while not finalized_response:
    ...
    log_activity("assistant", assistant_response)
    messages.append({"role": "assistant", "content": assistant_response})

    st.write(assistant_response)

    if not re.compile(r"\?$").search(assistant_response):
        break

    user_feedback = st.text_input("Feedback", key="user_feedback" + str(count))
    count+=1
...
log_activity("system", final_instruction)

try:
    function_call = client.chat.completions.create(
        model="gpt-3.5-turbo", messages=messages, max_tokens=500
